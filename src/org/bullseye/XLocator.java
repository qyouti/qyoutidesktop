 ...
 * @author jon
 */
public class XLocator extends Thread
{
  int debuglevel=0;
  int maxwidth;
  int maxheight;
  ...
  static final double SOBEL_THRESHOLD = 0.45; //0.45;
   ...
   */
  public XLocator( int maxwidth, int maxheight )
  {
    int i, j;
  ...
  void notifyListeners( int percentage, boolean complete, BufferedImage debugimage )
  {
    ...
    while ( iter.hasNext() )
    {
      ...
      if ( debugimage != null ) listener.notifyNewDebugImage( debugimage, currentinput );
...
//    trigger = true;
  ...
  void processAllVotes( int corner )
  {
    ...
    if ( debuglevel >= 3 )
      notifyListeners(-1, false, votesToImage( cornervotemap[corner], maxvote ) );
    ...
    if ( debuglevel >= 3 )
    {
      ...
      notifyListeners(-1, false, smoothvotemapimage );
    }    
  }
      ...
      for (  iwest=0;  iwest<localmaxima[1].size();  iwest++ )
        for ( inorth=0; inorth<localmaxima[2].size(); inorth++ )
          for (  ieast=0;  ieast<localmaxima[3].size();  ieast++ )
          {
            ...
            currentreport.angle = Math.atan2( ps.y - intercept.y, ps.x - intercept.x )- 
                    Math.atan2( pw.y - intercept.y, pw.x - intercept.x );
            while ( currentreport.angle >  Math.PI ) currentreport.angle -= Math.PI;
            while ( currentreport.angle < -Math.PI ) currentreport.angle += Math.PI;
            // don't care about direction of the angle...
            currentreport.angle = Math.abs( currentreport.angle );
            // want the acute angle
            if ( currentreport.angle > Math.PI/2.0 )
              currentreport.angle = Math.PI - currentreport.angle;
            System.out.println( "currentreport.angle " + currentreport.angle );
            if ( currentreport.angle < Math.PI/4 )
            {
            ...
            currentreport.lineratio = Math.min( nslen, ewlen ) / Math.max( nslen, ewlen );
            System.out.println( "currentreport.lineratio " + currentreport.lineratio );
            if ( currentreport.lineratio < 0.25 )
            {
            ...
            currentreport.northsouthbias = Math.abs( nclen - nslen/2 ) / (nslen/2);
            System.out.println( "currentreport.northsouthbias " + currentreport.northsouthbias );
            if ( currentreport.northsouthbias > 0.75 )
            {
              System.out.println( "Intercept too far from middle of north-south line" );
              continue;
            }
            currentreport.eastwestbias = Math.abs( wclen - ewlen/2 ) / (ewlen/2);
            System.out.println( "currentreport.eastwestbias " + currentreport.eastwestbias );
            if ( currentreport.eastwestbias > 0.75 )
            {
            ...
            if ( score > currentreport.xQuality )
            {
              currentreport.xQuality = score;
              bestperm[0] = isouth;
              bestperm[1] = iwest;
              bestperm[2] = inorth;
              bestperm[3] = ieast;
  ...
  public void pass() // PassResult result )
  {
    ...
    if ( debuglevel >= 2 )
      notifyListeners( -1, false, sobelresult.toImage() );
    ...
    if ( debuglevel >= 3 )
    {
      for ( edge=0; edge<4; edge++ )
      {
        ...
        notifyListeners(-1, false, votesToImage( votemapa[edge], maxvote ) );
        voteStats( votemapb[edge] );
        notifyListeners(-1, false, votesToImage( votemapb[edge], maxvote ) );
      }
    }
  ...
  void sleepSeconds( int sec )
  {
    ...
    {
      Logger.getLogger(XLocator.class.getName() ).
  ...
  public void run()
  {
    ...
    for ( currentinput=0; currentinput<inputlist.length; currentinput++ )
    {
      ...
      prefilter();
      if ( debuglevel >= 1 )
        notifyListeners(-1, false, input );
      currentreport = new XLocatorReport();
      currentreport.image = input;
      locateX();
      inputlist[currentinput] = null;
  
      notifyListeners( 100*currentinput/inputlist.length, true, null );
    }
    
    notifyListeners( 100, false, null );
  }
